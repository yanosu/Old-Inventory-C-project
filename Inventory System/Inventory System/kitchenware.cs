...
namespace Inventory_System
{
    public partial class kitchenware : Form
    {
        ...
        private void DisplayData()
        {
            ...
            da = new OleDbDataAdapter("select * from db_users.KitchenWare", con);
            da.Fill(dt);
        ...
        {
            string query = "SELECT * FROM KitchenWare WHERE ID LIKE @searchValue OR ItemName LIKE @searchValue OR Amount LIKE @searchValue OR Record_Date LIKE @searchValue";
        ...
        private void insertbtn_Click(object sender, EventArgs e)
        {
            if (itemnametbox.Text != "" && itemamountbtox.Text != "")
            {
                ...
                con.Open();
                cmd = new OleDbCommand("INSERT INTO db_users.KitchenWare(ItemName, Amount, Record_Date) VALUES (@itemname, @amount, @date)", con);
        ...
        private void updatebtn_Click(object sender, EventArgs e)
        {
            if (itemamountbtox.Text != "" && itemnametbox.Text != "" && txtID.Text != "")
            {
                ...
                {
                    con.Open();
                    cmd = new OleDbCommand("UPDATE KitchenWare SET ItemName=?, Amount=? WHERE ID=?", con);
        ...
        private void deletebtn_Click(object sender, EventArgs e)
        {
            if (itemamountbtox.Text != "" && itemnametbox.Text != "" && txtID.Text != "")
            {
                cmd = new OleDbCommand("DELETE FROM KitchenWare WHERE ID=?", con);
                con.Open();
                cmd.Parameters.AddWithValue("@id", txtID.Text);
                cmd.ExecuteNonQuery();
                con.Close();
